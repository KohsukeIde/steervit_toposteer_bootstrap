...
def canonicalize_attribute_name(raw: str | None) -> dict[str, str] | None:
...
def record_attribute_types(record: dict[str, Any]) -> set[str]:
    return {x["type"] for x in extract_canonical_attrs(record)}


def is_color_bearing(record: dict[str, Any]) -> bool:
    return "color" in record_attribute_types(record)
